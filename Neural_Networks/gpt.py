...
class MultiHeadBiagramLanguageModel(nn.Module):
    ...
    def __init__(
        self, vocab_size, context_size, emb_size, head_size, head_dim, layer_size
    ):
        super().__init__()  # call the superclass to inherit its methods
        self.context_size = context_size
        # Creates embedding layers.
        ...
        # Iterates over multiple instances of the transformer
        self.blocks = nn.Sequential(
            *[
                Block(context_size, emb_size, head_size, head_dim, head_size)
    ...
    def forward(self, input, target=None):
        ...
        pos_emb = self.position_emb_table(torch.arange(T))  # (T,C)
        x = tk_emb + pos_emb  # (B,T,C)
        # Apply multi-head self-attention and FeedForward
        x = self.blocks(x)
        # Apply layer normalization prior the last linear layer
        x = self.ln(x)
        # Output of the NN
        logits = self.lm_nn(x)  # This creates a (B,T,vocab size) tensor
        # Compute the loss
...
class MultiHead(nn.Module):
    ...
    def __init__(self, context_size, emb_size, head_size, head_dim):
        ...
        self.proj = nn.Linear(head_size * head_dim, head_size * head_dim)
        self.dropout = nn.Dropout(dropout)

    def forward(self, input):
        # Concatenate the output of the multiple head modules over the channel dimension.
        input = torch.cat([h(input) for h in self.head_list], dim=2)
        input = self.dropout(self.proj(input))
        return input


class FeedForward(nn.Module):
    """Create a MLP module to generate competition between the nodes."""

    def __init__(self, ff_size):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(ff_size, 4 * ff_size),
            nn.ReLU(),
            nn.Linear(4 * ff_size, ff_size),
            nn.Dropout(dropout),
        )
...
class Block(nn.Module):
    """
    Create a Transformer block. Head allows communication, which is followed by
    the competition generated by the Linear Layer.
    """

    def __init__(self, context_size, emb_size, head_size, head_dim, ff_size):
        super().__init__()
        mhead_size = head_size // head_dim
        self.heads = MultiHead(context_size, emb_size, mhead_size, head_dim)
        self.ff = FeedForward(ff_size=ff_size)
        self.ln1 = nn.LayerNorm(normalized_shape=emb_size)
        self.ln2 = nn.LayerNorm(normalized_shape=head_size)

    def forward(self, input):
        # Communication
        input = input + self.heads(self.ln1(input))
        # Competition
        input = input + self.ff(self.ln2(input))
        return input
